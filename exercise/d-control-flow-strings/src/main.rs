...
fn main() {
    ...
    for arg in args {
        ...
        //
        // - [ x ] If arg is "sum", then call the sum() function
        // - [ x ] If arg is "double", then call the double() function
        // - [ x ] If arg is anything else, then call the count() function, passing "arg" to it.
...
fn count(arg: String) {
    ...
    // There is a specialty type known as a label which is a reference
    // to the uncon loop, the reference is later referenced to be broke
    // on line 81
    'eight_speaker: loop {
        ...
        if count == 8{ break 'eight_speaker; }
    }
